...
void oset_insert(OrderedSet oset, void* key, void* value) {
    ...
    if (new_node->forward[0] != OSET_EOF) new_node->forward[0]->previous = new_node;
...
OrderedSet oset_split(OrderedSet oset, void* split_key) {
    ...
    if (split->max_level < oset->max_level) {
        ...
        split->header->forward =
            calloc(split->max_level * sizeof(*split->header->forward), split->max_level);
    ...
    if (split->header->forward[0] != OSET_EOF) split->header->forward[0]->previous = split->header;

    // Update first and last pointers of SPLIT.
    split->first = split->header->forward[0];
    if (split->first != OSET_EOF) split->last = oset->last;  // Atleast one element is in SPLIT.
...
OrderedSet oset_merge(OrderedSet a, OrderedSet b) {
    ...
    if (a == OSET_ERROR) return OSET_ERROR;
    ...
    if (merged->max_level < max(a->max_level, b->max_level)) {
        ...
        // Reallocate and initialize to NULL the forward array of MERGED header node.
        free(merged->header->forward);
        merged->header->forward =
            calloc(merged->max_level * sizeof(*merged->header->forward), merged->max_level);
        if (merged->header->forward == NULL) return OSET_ERROR;
    }
    ...
    for (int i = 0; i < merged->header->levels; i++) vector_set_at(last_nodes, i, merged->header);

    // Transfer nodes from A and B to MERGED.
    while (a->header->forward[0] != OSET_EOF && b->header->forward[0] != OSET_EOF) {
        ...
        do {
            ...
            if (lvl == 0 && node->forward[0] != OSET_EOF) node->forward[0]->previous = node;
            lvl++;
        } while (
            lvl < merged->header->levels &&
            a->header->forward[lvl] != OSET_EOF &&
            merged->compare(a->header->forward[lvl]->key, key2) < 0);
        lvl--;

        // For each level attached to MERGED, find endpoint at that level and remove it from A.
...
void oset_concat(OrderedSet a, OrderedSet b) {
    ...
    // Connect previous pointer of first node of B to last node of A to be able to traverse A in
    // descending order.
    if (b->first != NULL) b->first->previous = a->last;
